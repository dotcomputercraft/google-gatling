...
class GoogleSimulation extends Simulation {
  ...
  val rest_api = "http://" + System.getProperty("rest_api_url", "www.google.com")
  println("URL host is: " + rest_api)

  val noOfUsers: Int = Integer.getInteger("users", 100).intValue
  println("noOfUsers = " + noOfUsers)

  val rampup = Integer.getInteger("rampup", 75).intValue
